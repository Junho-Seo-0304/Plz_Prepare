...
class OrderStateMenuAdapter(val ctxt : Context, val layoutId : Int, val RouteList : List<CheckingRoute>)
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
        ...
        firebase.addValueEventListener(object : ValueEventListener{
            ...
            override fun onDataChange(p0: DataSnapshot) {
                if(p0.child("PermissionOrder").child(RouteList[position].Number.toString()).child("Customer").value==mAuth.currentUser!!.uid){
                    Rname.text=p0.child("rname").value.toString()
                    StateText.text="수락대기중"
                    Number.text=RouteList[position].Number.toString()
                    var totalString =""
                    for(i in 1 until p0.child("PermissionOrder").child(RouteList[position].Number.toString()).childrenCount-1){
                        totalString += p0.child("PermissionOrder").child(RouteList[position].Number.toString()).child((i-1).toString()).child("food").child("fname").toString()+" : "+p0.child("PermissionOrder").child(RouteList[position].Number.toString()).child((i-1).toString()).child("num").toString()+"\n"
                    }
                    Orderlist.text=totalString
                    state="Permission"
                }
                else if(p0.child("ReadyOrder").child(RouteList[position].Number.toString()).child("Customer").value==mAuth.currentUser!!.uid){
                    Rname.text=p0.child("rname").value.toString()
                    StateText.text="수락 완료"
                    Number.text=RouteList[position].Number.toString()
                    var totalString =""
                    for(i in 1 until p0.child("ReadyOrder").child(RouteList[position].Number.toString()).childrenCount-1){
                        totalString += p0.child("ReadyOrder").child(RouteList[position].Number.toString()).child((i-1).toString()).child("food").child("fname").toString()+" : "+p0.child("ReadyOrder").child(RouteList[position].Number.toString()).child((i-1).toString()).child("num").toString()+"\n"
                    }
                    Orderlist.text=totalString
                    state="Ready"
                }
